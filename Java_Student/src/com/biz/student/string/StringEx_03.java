...
public class StringEx_03 {
	...
	public static void main(String[] args) {
		...
		strNations = strNation.split(""); // 문자열을 알파벳 기준으로 나눠라

		for(String s : strNations) {
			System.out.print(s+"\t");
		}
		System.out.println();
		strNations = strNation.split(" "); // space(빈칸) 기준으로 나눠라 문자열
		...
		System.out.println();
		System.out.println();
		String score ="1,53,89,59,201,67";
		...
		/*
		 * score.csv 파일을 읽어서 코드를 검증하고 모든 결과가 일치로 나타나는 지 판단을 하는 것을
		 * 단위(모듈) TEST 라고 한다.
		 * 이때 사용된 score.csv 파일의 데이터를 TESTCASE 라고 한다.
		 * 
		 */

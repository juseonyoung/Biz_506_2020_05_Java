...
public class BankEx_04 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			/*
			 * interface를 설계할 당시 필요하지 않아 정의하지 않은 method 는
			 * 보통 구현된 class 내에서만 사용하는 것으로 private 또는 protected로 선언한다.
			 * 그러나 프로젝트가 진행되는 동안 구현된 class에 어떤 메서드를 추가하고 
			 * 이 method를 외부에서 호출하여 어떤 행동을 수행해야 할 필요가 있을 때가 있음!
			 * 
			 * 객체를 선언할 때 interface로 선언을 하면
			 * 클래스에 구현된 method를 직접 호출하는 것이 안된다
			 * 이럴 때는 down casting을 해서 해당 method 를 사용할 수 있다.
			 * 
			 * 
			 */
			boolean bRet =((BankServiceImplV3)bService).account(); // 호출되지 않는 메서드
			//interface되어있지 않으므로..
